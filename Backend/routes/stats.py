...
def average_connectivity_time_per_operator():
    ...
    result = [{'operator': operator, 'average_connectivity_time': max(avg_time, -avg_time) if avg_time and avg_time < 0 else avg_time} for operator, avg_time in operator_stats]
...
def average_connectivity_time_per_network_type():
    ...
    result = [{'network_type': network, 'average_connectivity_time': max(avg_time, -avg_time) if avg_time and avg_time < 0 else avg_time} for network, avg_time in network_stats]
    return jsonify(result), 200
...
def average_signal_power_per_network_type():
    current_user_id = get_jwt_identity()
    signal_power_stats = db.session.query(
...
def average_snr_or_sinr_per_network_type():
    current_user_id = get_jwt_identity()
    snr_sinr_stats = db.session.query(
        CellData.network_type, func.avg(CellData.sinr)
    ).filter(
        CellData.user_id == current_user_id,
        CellData.sinr.isnot(None)
    ).group_by(CellData.network_type).all()
    result = [{'network_type': network, 'average_sinr': max(avg_sinr, -avg_sinr) if avg_sinr and avg_sinr < 0 else avg_sinr} for network, avg_sinr in snr_sinr_stats]
    return jsonify(result), 200
